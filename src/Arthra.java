...
public class Arthra {
	...
	private static String[][] articleReq() {
		return retArticleList();
	}
	private static String[][] retArticleList() {
		String[][] aList = new String[20][4];
		for (int loopIndex = 0; loopIndex < 20; loopIndex++) {
		      aList[loopIndex][0] = "Τίτλος"  + loopIndex;
		      aList[loopIndex][1] = "http://www.moh.gov.gr/";
		}
		return aList;
	}
	...
	protected void createContents() {
	    ...
	    String[] titles = {"Τίτλος Άρθρου", "Link" };
	    ...
	    String[][] aList = articleReq();
	    for (int loopIndex = 0; loopIndex < 20; loopIndex++) {
	      TableItem item = new TableItem(table, SWT.NULL);
	      item.setText(0, aList[loopIndex][0]);
	      item.setText(1, aList[loopIndex][1]);	      
	    }
	    ...
	    table.setBounds(281, 136, 325, 204);
	    
	    TableCursor tableCursor = new TableCursor(table, SWT.NONE);
	    tableCursor.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				if(tableCursor.getColumn()== 1) {
					Program.launch(tableCursor.getRow().getText(1));
					tableCursor.getRow().setForeground(0,SWTResourceManager.getColor(SWT.COLOR_MAGENTA));
					tableCursor.getRow().setForeground(1,SWTResourceManager.getColor(SWT.COLOR_MAGENTA));
				} 
			}
	    });
